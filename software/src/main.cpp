...
static inline void usage(char *const proc) {
    fprintf(stderr,
            "Usage: %s -N <num_of_queues (should be <= 8)>"
...
static int parse_cmd_options(int argc, char *const argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "N:d:b:", long_options,
                              &option_index)) != -1) {
        switch (opt) {
            case 0:
                if (strcmp(long_options[option_index].name, "workload") == 0) {
                    snprintf(g_args.workload_spec_file,
                             sizeof(g_args.workload_spec_file), "%s", optarg);
                } else if (strcmp(long_options[option_index].name,
                                  "rsr_conf") == 0) {
                    snprintf(g_args.rsr_conf_file, sizeof(g_args.rsr_conf_file),
                             "%s", optarg);
                } else {
                    ret = -1;
                }
            ...
            case 'N':
                ...
                break;
            case 'd':
                snprintf(g_args.pcie_dev, sizeof(g_args.pcie_dev), "%s",
                         optarg);
                break;
            case 'b':
...
int main(int argc, char **argv) {
    ...
    info("creating eth device: %s, #queues: %d", g_args.pcie_dev,
         g_args.num_queues);
    if (g_args.num_queues == 12) {
        mqnic_eth_dev_create(&eth_dev, g_args.pcie_dev, g_args.num_queues, 15);
    } else {
        mqnic_eth_dev_create(&eth_dev, g_args.pcie_dev, g_args.num_queues,
                             g_args.num_queues - 1);
    }
    ...
    for (auto i = 0; i < kMaxNumLCores; i++) {
        ...
        struct rte_mempool *mp = rte_pktmbuf_pool_create(
            mp_name, 8196, 256, 0, MAX_PKT_SIZE + RTE_PKTMBUF_HEADROOM,
            rte_socket_id());
        if (NULL == mp) {
    ...
    info("start, i am running with QN");

    struct qn_ctxt qn_ctxts[kMaxNumLCores];
    for (auto i = 0; i < lcore_cnt; i++) {
        ...
        qn_ctxts[i].db = db;
        //
        rte_eal_remote_launch(qn_echo_proc, (void *)&(qn_ctxts[i]),
                              lcores[i]);
    }
